                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div class="col-lg-8">
                                ...
                                <p class="text-muted font-13">Do something with all data rute.
                                </p>
                            </div>
                            <a class="btn btn-primary btn-sm" href="{{ route('route.create') }}"><i
                                    class="fas fa-fw fa-plus text-sm"></i> Add new route</a>
                        </div>
                        ...
                        @if(count($routes) < 1)
                            <div class="text-center">
                                ...
                                <small>It looks like route data is still empty, add or create new a route data 
                                    immediately!</small>
                            </div>
                        @else
                            ...
                            <tbody>
                                @foreach ($routes as $item)
                                    <tr>
                                        ...
                                        <td>{{ MyHelpers::changeIntegertoRupiah($item->harga_carteran) }}</td>
                                        <td>{{ $item->jam_pemberangkatan }} </td>
                                        <td>{{ MyHelpers::ubahFormatTimestamp($item->created_at) }}</td>
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                <a class="btn btn-sm text-info text-small me-1"
                                                    href="{{ route('route.edit', MyHelpers::encode($item->id)) }}">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </a>
                                                <form action="{{ route('route.destroy', $item->id) }}" method="post"
                                                    id="delete-route{{ $item->id }}" style="display: inline;">
                                                    @method('delete')
                                                    @csrf
